...
public class Keypad {
        ...
        switch (dotsCount(inputString)) {
            ...
            case 1: // 1 dots -> 
                return switch (dotsPosition(inputString)) {
                    case 1 ->
                        2; //dot locate between start and end -> float
                    case 2 ->
                        2; //dot locate at end -> no decimal stuff -> float to avoid error
                    default ->
                        0; //dot locate at start -> not numeric -> invalid
                };
    ...
    public double StringtoDouble(String inputString) {
        if (IntegerOrDouble(inputString) != 0) {
            BigDecimal bd = new BigDecimal(inputString).setScale(2, RoundingMode.DOWN);
            return bd.doubleValue();
        } else {
